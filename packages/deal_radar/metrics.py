...
def snapshot() -> dict:
    ...
    for k, s in _lat_sum.items():
        n = _lat_n[k] or 1
        lat[k] = round(s / n * 1000, 2)
    return {"counters": dict(_counters), "gauges": dict(_gauges), "latency_avg_ms": lat}
...
def prometheus() -> str:
    ...
    for gk, gv in _gauges.items():
        lines.append(f"dealradar_{gk} {gv}")
    for lk, ls in _lat_sum.items():
        ln = _lat_n[lk] or 1
        lines.append(f"dealradar_{lk}_avg_ms {ls / ln * 1000:.2f}")
